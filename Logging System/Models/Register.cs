...
namespace Logging_System.Models
{
    public class Register
    {
        ...
        public string InsertRegDetails(Register _lean)
        {
            ...
            int randomN;
            randomN = rr.Next(123, 21344);
            _lean.MentorID = randomN;


            string randomRNS;

            randomRNS = ((alphas[rr.Next(0, 52)] + rr.Next(0, 111) + alphas[rr.Next(0, 52)] + rr.Next(0, 300) + alphas[rr.Next(0, 52)] + rr.Next(0, 200) + alphas[rr.Next(0, 52)]));
            _lean.RSAID = randomRNS;
            
            cmd.CommandText = "Insert into [MentorsCredentials] values('" + _lean.MentorID
                 + "','" + _lean.FirstName
                + "','" + _lean.MiddleName
                + "','" + _lean.Surname
                + "','" + _lean.Gender
            ...
            string Body = ("Hi " + _lean.FirstName + " " + _lean.MiddleName + " " + _lean.Surname + " ,                                                                                                   this is to confirm that you have been registered on the Learners Log System, as a Mentor. Your login details for the Log System are " + "Username: " + _lean.Username + "  Password: " + randomPassword + " Keep the following Confidential Password Retrieval Code to reset your password " + randomRNS + " .Please NOTE that you are permitted change your Password. For any queries please send an email to logsystemsupp@gmail.com. Login to http://learnerslogsystem.azurewebsites.net/Mentor/Login");
            mail.Body = Body;
